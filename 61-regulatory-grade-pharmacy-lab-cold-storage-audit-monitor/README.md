...
| [SparkFun TMP117 High Precision Temperature Sensor — Qwiic (SEN-15805)](https://www.sparkfun.com/products/15805) | 1 | **Bench substitute only — not the production sensor path.** ±0.1°C accuracy, I²C via Qwiic. Using TMP117 requires swapping the `Adafruit_MAX31865` library for `SparkFun_TMP117` and replacing `readTemperatureC()` in `firmware/cold_storage_audit_monitor_helpers.cpp`. Mounts inside the enclosure; measures exterior ambient air only. Does not ship with a NIST-traceable calibration certificate. Not a deployable compliance instrument. |
...
| File | Contents |
|---|---|
| [`firmware/cold_storage_audit_monitor.ino`](firmware/cold_storage_audit_monitor.ino) | Entry points (`setup`, `loop`), Notecard configuration, template definition, and the per-wake sample cycle |
| [`firmware/cold_storage_audit_monitor_helpers.h`](firmware/cold_storage_audit_monitor_helpers.h) | `AppState` struct, shared `#define` constants, `extern` globals, and helper function prototypes |
| [`firmware/cold_storage_audit_monitor_helpers.cpp`](firmware/cold_storage_audit_monitor_helpers.cpp) | Sensor reads, env-var parsing, `sendReading`, `sendAlert`, and `goToSleep` implementations |

> **Build note.** Arduino IDE requires that a `.ino` sketch reside in a folder whose name matches the sketch. Opening `firmware/cold_storage_audit_monitor.ino` in the IDE will prompt you to move it — click **OK** to let the IDE create the matching subfolder. `arduino-cli compile` does not impose this constraint and builds directly from the `firmware/` directory.
