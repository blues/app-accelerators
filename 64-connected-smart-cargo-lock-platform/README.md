...
![Connected Smart Cargo Lock Platform banner](banner.png)

<Note>

This reference application is intended to provide inspiration and help you get started quickly. It uses specific hardware choices that may not match your own implementation. Focus on the sections most relevant to your use case. If you'd like to discuss your project and whether it's a good fit for Blues, [feel free to reach out](https://blues.com/landing-pages/accelerators-contact-us/?accelerator=Connected%20Smart%20Cargo%20Lock%20Platform).

</Note>

This project is a [loss prevention](https://blues.com/loss-prevention/) reference platform for connected smart cargo locks. The lock fires an immediate alert the moment the shackle is pulled, the bolt is released, or someone starts grinding on the lock body while it is engaged, and it does so wherever the cargo happens to be, whether that's a strip-mall drop lot, an interstate at 3 AM, or a container ship mid-Atlantic. The hardware (a [Notecard for Skylo](https://shop.blues.com/products/notecard?utm_source=dev-blues&utm_medium=web&utm_campaign=store-link) that combines global cellular with satellite failover in a single module, paired with a reed shackle sensor, a hall-effect bolt detector, and the Notecard's onboard accelerometer for tamper scoring) lives inside the lock body and runs for multiple months on a 5,000 mAh cell under cellular-dominant operation. See [§6](#low-power-strategy) for the energy budget and validate with Mojo before sizing for production.
...
**Device-side responsibilities.** Every minute, the onboard Cygnet STM32 host on the Notecarrier CX wakes for a few seconds, reads the reed switch and hall-effect sensor, queries the Notecard's built-in accelerometer for the last minute's motion count, runs the lock state machine, and queues any resulting Notes over I²C. The rest of the minute the host is completely off — cut by the Notecard's `card.attn` sleep mechanism — and so is the +3V3 rail that powers the SS461A and its pull-up resistors. That power-cut sleep is what makes a 5,000 mAh cell last multiple months on this design. Runtime state persists across sleeps via [`NotePayloadSaveAndSleep`](https://dev.blues.io/guides-and-tutorials/notecard-guides/feather-mcu-low-power-management/) and is restored via `NotePayloadRetrieveAfterSleep` on each fresh `setup()`.

**Notecard responsibilities.** The Notecard for Skylo handles all three radios — LTE-M, NB-IoT, and Skylo NTN satellite — and keeps the firmware out of carrier negotiations entirely. It buffers each [Note](https://dev.blues.io/api-reference/glossary/#note) in its on-device queue, opens a scheduled session on the [`hub.set`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) `outbound` cadence (default 6 hours), and treats any `sync:true` alert Note as an immediate uplink — the lock comes off in a parking lot, and the dispatcher hears about it before the thief is off the property. [Environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) flow back on each inbound sync, letting a fleet manager retune tamper thresholds for a pharmaceutical load without ever touching a single device.

**Notehub responsibilities.** Each lock's embedded global SIM lets the Notecard authenticate against any supported carrier worldwide and deliver data to [Notehub](https://dev.blues.io/notehub/notehub-walkthrough/) over the Internet. Notehub stores every event and applies the project's [routes](https://dev.blues.io/notehub/notehub-walkthrough/#routing-data-with-notehub). Immediate lock events (`lock_event.qo`) land in one Notefile and 6-hour status summaries (`lock_status.qo`) in another, so a real-time dispatch system and a long-term audit log can be served from the same device with no per-Note filter logic in between. [Smart Fleets](https://dev.blues.io/notehub/notehub-walkthrough/#using-smart-fleet-rules) let a shipper push tighter thresholds to the high-value pharmaceutical fleet while leaving the dry-goods fleet untouched — same firmware, different policy.
...
3. **Flash.** In Arduino IDE: File → Open → `firmware/cargo_lock/` → under **Tools → Board → STMicroelectronics STM32 → Blues**, select **Cygnet (Notecarrier CX)** → Upload. Or via `arduino-cli` (the FQBN below matches `firmware/cargo_lock/sketch.yaml`, so omitting `--fqbn` also works when invoked from the sketch directory):
   ```bash
   arduino-cli compile --fqbn STMicroelectronics:stm32:Blues:pnum=CYGNET -u -p /dev/ttyACM0 firmware/cargo_lock/
   ```
...
The whole lock body has to fit a Notecarrier CX plus Notecard, two sensors, a 5,000 mAh LiPo, and a single skyward-facing antenna into a 120 × 65 × 40 mm polycarbonate enclosure that mounts behind the shackle. Every host I/O lands on the [Notecarrier CX](https://dev.blues.io/datasheets/notecarrier-datasheet/notecarrier-cx-v1-3/) dual 16-pin header, and the Notecard for Skylo (NOTE-NBGLWX) seats in the carrier's M.2 slot. During bench validation the Mojo splices inline between the Li-Po positive terminal and the Notecarrier's `+VBAT` pad to read total system current; it comes out before the lock ships.

**Pin-by-pin:**

- **+3V3** → top of each 10 kΩ pull-up resistor (for D5 and D6), and SS461A VCC with 100 nF bypass cap to GND. **This is the ATTN-gated host rail.** The Notecarrier CX routes the Notecard's ATTN signal to the Cygnet power enable; when `NotePayloadSaveAndSleep` issues its sleep request, this rail is removed entirely for the sleep period — the SS461A (~4–8 mA quiescent at 3.3 V) and pull-up resistors draw zero standby current between wakes. Verify the rail is being cut as expected with Mojo as described in [§9](#9-validation-and-testing).
...
The NOTE-NBGLWX exposes a `MAIN` u.FL connector for cellular and Skylo satellite. **Use the Skylo-certified MAIN antenna included with the Notecard for Skylo** — see [§4](#4-hardware-requirements) for the certification and substitution requirements.
...
The firmware is small on purpose — every wake runs the same compact loop of sense, evaluate, queue, sleep — but the lock state machine and tamper-cooldown handling need enough room to read clearly, so the sketch is split into three files under [`firmware/cargo_lock/`](firmware/cargo_lock/): [`cargo_lock.ino`](firmware/cargo_lock/cargo_lock.ino) (entry point — `setup()`, `loop()`, user-facing constants), [`cargo_lock_helpers.h`](firmware/cargo_lock/cargo_lock_helpers.h) (enums, `PersistState` struct, function prototypes), and [`cargo_lock_helpers.cpp`](firmware/cargo_lock/cargo_lock_helpers.cpp) (Notecard config, env-var handling, sensor reads, state machine). Open the `firmware/cargo_lock/` folder in the Arduino IDE or point `arduino-cli` at it.
...
Assume 1,440 host wakes per 24 hours (every 60 s) and four scheduled radio sessions per day (6-hour cadence). With a ~20 mA average during each 2-second active wake window (Cygnet + SS461A quiescent + pull-up bleed + Notecard I²C servicing), 50 mA average during each LTE-M session (~5 minutes including radio startup, registration, and data transmission), and the Notecard's documented ~8 µA idle between sessions, a typical 24-hour profile under strong cellular coverage works out to:
- Host wakes: 1,440 × 20 mA × 2 s = 57,600 mA·s ÷ 3,600 ≈ **~16 mAh per 24 h**
- Cellular sessions: 4 × 50 mA × 5 min = 60,000 mA·s ÷ 3,600 ≈ **~17 mAh per 24 h**
- Notecard idle (8 µA × ~24 h): ≈ **~0.2 mAh per 24 h**
- **Total: ~33 mAh per 24 h on a cellular-dominant network**

With a 5,000 mAh cell, this projects to roughly **~150 days (~5 months)** of battery life under ideal conditions — **multi-month**, not literally a year. Reaching a true 12-month figure on this cell would require either reducing the radio session count below ~3 per day or substantially shrinking the host-wake duty cycle (the two dominant terms above), and is outside the scope of the default cadence shown here. **Satellite-dominant deployments draw materially more energy per session — both higher current and longer link establishment than LTE-M — and will reduce endurance substantially.** This calculation assumes no open/tamper events (which force immediate `sync:true` sessions); each unscheduled session adds approximately one cellular-session-worth of energy. The active-wake current and duration assumed here are illustrative — the only authoritative numbers come from a Mojo bench soak in your specific deployment environment.
...
**Use Mojo bench measurements** in your specific deployment environment to establish a ground-truth figure before committing to a cell size. The standby current between wakes should reflect only the Notecard's idle current (~8 µA) — not the SS461A's ~4–8 mA quiescent draw. If you see continuous milliamp-level drain between sensor wakes, the +3V3 rail may not be gated by ATTN as expected — confirm that the SS461A VCC and pull-up top rails are wired to the ATTN-gated +3V3 pin rather than an always-on supply, and see [§9](#9-validation-and-testing) for the full Mojo validation procedure.
...
2. **Validate the sensor rail sleep assumption.** Between host wakes, the Mojo should show only the Notecard's idle current (~8 µA). If you see a continuous ~4–8 mA baseline between wakes, the SS461A and/or pull-up resistors are drawing current — the +3V3 rail is not being cut by ATTN as expected. Confirm that the SS461A VCC and pull-up top rails are wired to the ATTN-gated +3V3 pin (see [§5](#5-wiring-and-assembly)) and not to an always-on supply rail.
...
| Device claims fine but no Notes appear in Notehub | Notecard hasn't synced to cellular/satellite yet | First sync can take 30–60 seconds on cellular or 2–5 minutes on satellite. Check Notehub project Events tab for device last-connect time. If stuck >10 minutes, check antenna u.FL connection and verify the enclosure lid faces skyward. |
...
| Shackle or bolt state doesn't change when physically actuated | Sensor wiring reversed or pin short | Verify D5 and D6 are wired to the correct terminals (see [§5](#5-wiring-and-assembly) pin table). Test with `digitalWrite(PIN_SHACKLE, LOW); delay(100); digitalWrite(PIN_SHACKLE, HIGH);` in a bench sketch to confirm the GPIO responds. |
...
| Mojo shows 4–8 mA baseline between wakes instead of ~8 µA | +3V3 rail (SS461A, pull-ups) not gated by ATTN | Verify the SS461A VCC and pull-up top rails are wired to the ATTN-gated +3V3 pin (see [§5](#5-wiring-and-assembly)) and not to an always-on rail. |
...
This reference platform demonstrates the connectivity story, the sensor stack, and the state machine — but it deliberately stops short of being a finished commercial cargo lock. A lock OEM moving toward production will want interrupt-driven open detection, BLE driver authentication, and a tamper scorer that uses spectral analysis rather than a simple motion count. Those are the items below, framed as scope boundaries for the POC and the engineering work that turns it into shippable product.
...
Cargo theft is a detection problem masquerading as a security problem — a modern lock is strong enough; what it lacks is the ability to raise an alarm while someone is still grinding on it, or to confirm that the container that left Rotterdam is still sealed when it arrives in Newark. The fleet manager who used to find out about a stolen load when the driver opened the trailer at the destination now gets an alert seconds after the shackle pops, wherever the cargo happens to be. Two digital sensors, the Notecard's onboard accelerometer, a 60-second duty cycle, and a single Notecard for Skylo that handles global cellular plus satellite failover without a carrier contract — that's the whole platform. The lock OEM ships one SKU worldwide, the fleet manager retunes thresholds per cargo class from a browser, and a 5,000 mAh cell lasts multiple months under cellular-dominant operation. For the next generation of connected cargo security, this is the architecture that makes it shippable.
