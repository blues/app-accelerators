...
4. **Set environment variables.** In Notehub, select a Fleet and click the **Environment** tab. All variables below are optional; firmware defaults are shown. Changes take effect on the device's next inbound sync — no firmware update needed.
   ...
   |---|---|---|
   | `sample_interval_sec` | `60` | Seconds between host wakeups (and sensor reads). The default 60 seconds matches the `card.motion` 1-minute accumulation bucket, ensuring every wake reads a fresh tamper score with no unobserved gap. Increase for longer battery life at the cost of a proportionally larger tamper blind window; valid range is 60–3600 s. |
   ...
   | `alert_cooldown_sec` | `1800` | Minimum seconds between repeated alerts of the same type. Prevents a sustained attack from flooding the operator's alert channel — one `tamper` alert per 30 minutes is enough. |
...
Assume 1,440 host wakes per 24 hours (every 60 seconds) and four scheduled radio sessions per day (6-hour cadence). With a ~20 mA average during each 2-second active wake window (Cygnet + SS461A quiescent + pull-up bleed + Notecard I²C servicing), 50 mA average during each LTE-M session (~5 minutes including radio startup, registration, and data transmission), and the Notecard's documented ~8 µA idle between sessions, a typical 24-hour profile under strong cellular coverage works out to:
- Host wakes: 1,440 × 20 mA × 2 seconds = 57,600 mA·s ÷ 3,600 ≈ **~16 mAh per 24 h**
- Cellular sessions: 4 × 50 mA × 5 minutes = 60,000 mA·s ÷ 3,600 ≈ **~17 mAh per 24 h**
...
Every `SAMPLE_INTERVAL_SEC` (default 60 seconds), the firmware reads three sensors, evaluates the lock state machine, and decides whether anything noteworthy happened:
...
> **Bench setup — `DEBUG_SERIAL` and power-cut sleep.** The firmware ships with `#define DEBUG_SERIAL` commented out; no log output appears until you uncomment that line and reflash. Note that enabling `DEBUG_SERIAL` adds up to a 3 seconds USB-enumeration wait on every wake, which measurably increases average power consumption — leave it commented in production builds.
>
...
3. Open the lock manually and confirm a `lock_event.qo` note with `"event":"opened"` appears in Notehub within the session-establishment window (~15–60 seconds on cellular, 2–5 minutes on satellite).
