...
**Notecard responsibilities.** The Notecard for Skylo stores [Notes](https://dev.blues.io/api-reference/glossary/#note) in its on-device queue, manages cellular and satellite radio selection automatically, and flushes the queue on the configured [`hub.set`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) outbound cadence — shorter when the trailer is in transit, longer during DC dwells. Alert notes with `sync:true` bypass the outbound timer and trigger an immediate radio session. The Notecard handles GNSS for asset tracking and tags every Note with the last known position. [Environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) pushed from Notehub reach the host on the next inbound sync — operators retune thresholds and sync cadences in the field without re-flashing firmware.

**Notehub responsibilities.** [Notehub](https://dev.blues.io/notehub/notehub-walkthrough/) terminates both cellular and satellite sessions, stores every event, and applies project-level [routes](https://dev.blues.io/notehub/notehub-walkthrough/#routing-data-with-notehub). Per-fleet [environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) let a single firmware image adapt to different cargo types — a frozen-goods customer and a produce customer differ only in the reefer temperature bands their fleet-level variables encode. [Smart Fleets](https://dev.blues.io/notehub/notehub-walkthrough/#using-smart-fleet-rules) let the OEM organize trailers by customer, cargo class, or route region without separate firmware builds.
...
| Part | Qty | Rationale |
|------|-----|-----------|
| [Notecarrier CX](https://shop.blues.com/products/notecarrier-cx?utm_source=dev-blues&utm_medium=web&utm_campaign=store-link) | 1 | Integrated carrier with embedded Cygnet STM32L433 host — no separate MCU needed. M.2 slot seats the Notecard for Skylo. |
...
| DC-DC converter, 9–36 V input, 5 V/3 A output ([Mean Well SD-15D-5](https://www.meanwell.com/webapp/product/search.aspx?prod=SD-15D-5)) | 1 | Steps down the 12 V supply (from tractor auxiliary line when connected, or reefer battery during DC dwells) to the 5 V rail powering the Notecarrier CX. The SD-15D-5's 36 V maximum input provides margin against the trailer's normal transient envelope above 12 V nominal. |
...
The following items are **not part of this reference build** and should not be purchased or assembled until the corresponding integration engineering is complete. The firmware includes stub parsers for both paths as placeholders; see [§9](#9-limitations-and-next-steps) for the full engineering scope required before either produces real trailer data.

| Part | Qty | Notes |
|------|-----|-------|
| **J2497 PLC modem custom board**: [Microchip (Yitran) IT700](https://www.microchip.com/en-us/product/IT700) PLC modem IC + Bourns PT60234PEL bus-coupling transformer + bus-clamping TVS diodes and ferrite filter on a custom PCB | 1 | No off-the-shelf UART-output J2497 module exists; the custom board resolves only the physical modem layer. Three additional engineering layers are required before any reefer data flows: (a) a licensed J2497 application-layer protocol stack, (b) reefer-OEM message mapping under a vendor agreement, and (c) field validation. See [§9](#9-limitations-and-next-steps) and [Future Integration: J2497](#future-integration-j2497-plc-modem-path). |
...
1. **Create a project.** Sign up at [notehub.io](https://notehub.io) and [create a project](https://dev.blues.io/quickstart/notecard-quickstart/notecard-and-notecarrier-pi/#set-up-notehub). Copy the [ProductUID](https://dev.blues.io/notehub/notehub-walkthrough/#finding-a-productuid) and paste it into `firmware/connected_trailer_platform/connected_trailer_platform.ino` as `PRODUCT_UID`.
...
The firmware spans three files:
...
- [`Blues Wireless Notecard`](https://github.com/blues/note-arduino) (the `note-arduino` library, **v1.8.5** at time of writing). Install via the Arduino Library Manager — search for `Blues Wireless Notecard`, select v1.8.5, and install — or via CLI: `arduino-cli lib install "Blues Wireless Notecard@1.8.5"`. See the [note-arduino releases page](https://github.com/blues/note-arduino/releases) for any update before building.
...
| Host awake — sample cycle + Notecard I²C calls | Bench-measure with Mojo — dominated by Cygnet STM32 active current; no Blues-published spec exists | ~1–3 s per `sampleIntervalSec` cycle |
...
DC-dwell battery sizing should be based on bench-measured Mojo data, not on component-level estimates. See [§8 Validation and Testing](#8-validation-and-testing) for the measurement procedure.

**UART acquisition during host-off sleep.** Both UART peripherals lose power when the host is off. On wakeup, `setup()` re-initialises `Serial1` (J2497) and `tpmsSerial` (TPMS) and then drains both channels for `WAKE_UART_DRAIN_MS` (250 ms) before the sample cycle. At 9600 baud, 250 ms accommodates ~3 complete J2497 frames. The host then stays awake for the duration of the sample cycle (typically 1–5 s including Notecard I²C calls), during which both channels are drained between each blocking call. For the J2497 path, frames arriving during a Notecard I²C call are captured in the STM32's 64-byte hardware UART ring buffer; loss is possible only if a single Notecard call takes longer than ~67 ms (full buffer at 9600 baud). For the TPMS path, `SoftwareSerial` requires an active CPU — it captures only bytes that arrive while the host is awake. TPMS gateways that buffer the most recent frame and re-transmit periodically are the most compatible; gateways that transmit only on pressure change may miss wakeup windows during stable-pressure dwell periods.
...
The Notecard for Skylo manages its own radio sleep independently: in [`periodic`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) mode it idles at ~8–18 µA between sessions, then wakes briefly for a cellular or satellite burst on the configured outbound cadence. The sync cadence switches automatically on every trailer state transition: `outbound_transit_min` (default 60 min) while moving, `outbound_parked_min` (default 240 min) during DC dwells. The host re-issues `hub.set` each time the cadence changes so the Notecard tracks the current schedule exactly.

Operators can extend `sample_interval_sec` via environment variable during long-dwell parked periods (for example, from 300 s to 900 s) to reduce the host's active-duty cycle when cargo temperature is stable.
...
- Alert de-duplication via `alert_cooldown_sec` (default 30 min per alert type) prevents a slowly-drifting temperature sensor from paging the fleet manager on every 5-minute sample cycle.
...
- Reefer setpoint and actual temperature (°F) parsed from a simplified POC UART frame — **integration-point stub; requires J2497 protocol stack and reefer-OEM mapping to produce real data** (see §9)
- Four tire-position pressures (PSI) parsed from a simplified POC UART packet — **integration-point stub; requires vendor-specific TPMS decode library** (see §9)
...
- **J2497 integration is a multi-layer development effort, not a wiring task.** The firmware models a simplified POC frame format, but a production J2497 data path has three distinct unresolved layers beyond building the custom IT700 coupling board: **(a) J2497 application-layer protocol stack** — the IT700 is a PLC modem IC, not a self-contained decoder; a software stack (typically licensed from Microchip/Yitran or a third-party integrator) must run above the modem IC to produce structured J2497 application frames on the host UART; **(b) reefer-OEM message discovery and mapping** — Thermo King and Carrier each implement proprietary J2497 message sets; the specific message IDs, data field offsets, temperature encodings, and status bits for a given reefer model are OEM-proprietary and must be obtained through a vendor integration agreement or a reverse-engineering effort under applicable terms; **(c) field validation on target reefer-unit hardware** — J560 bus content and J2497 message availability varies by manufacturer, model year, and configuration; validation must confirm that the desired temperature and status data is actually present and correctly mapped before treating this path as a working sensor input. Until all three layers are resolved for a specific reefer-unit model, this subsystem is an integration placeholder with a production data path that does not yet exist. **Alternative starting point:** many modern reefer units (Thermo King SLXe, Carrier X4 7700) expose temperature and status data over a dedicated RS-232 or RS-485 diagnostic port — mechanically simpler, better-documented at the protocol level, and a more realistic first milestone for an early prototype.
...
Refrigerated trailer telematics is contested territory — reefer OEMs and aftermarket platforms are already installed on every trailer competing for fleet-operator trust and recurring data revenue. A trailer manufacturer that wants its own connectivity footprint needs a platform on the trailer from day one, not a retrofit after the competition is already embedded. The Notecarrier CX with a Notecard for Skylo gives the OEM a single hardware SKU covering any regional market, any carrier network, and the satellite gaps that cellular leaves behind — with an embedded SIM and 10 years of included service so there's no mid-program re-sourcing conversation. This platform delivers three fully-implemented sensor paths (cargo-air temperature, door, GPS), includes firmware scaffolding and note-template fields for J2497 reefer telemetry and TPMS tire pressure as documented integration points awaiting the vendor engineering described in §9, and uses the Notecard's `card.attn` ATTN signal for dwell-capable host sleep — drawing only the Notecard's ~8–18 µA idle floor during long DC dwells. Notehub turns that raw telemetry into a programmable data bus: one API endpoint for the OEM's fleet management cloud, routable to any downstream system — cold-chain compliance archive, TPMS safety alert channel, GPS dispatch feed — without touching firmware. Over a 10–15 year trailer lifecycle, the combination of templated binary notes, state-based sync cadence switching, dwell-capable host sleep, and automatic cellular-to-satellite fallback keeps the platform running within the included data budget and without manual intervention, even in the coverage environments that knock out every cellular-only solution. That's the platform a trailer OEM needs to own the connectivity layer rather than cede it.
