...
5. **Set environment variables.** In Notehub, navigate to **Fleet → Environment** (or **Device → Environment** for a per-unit override). Add any of the variables below. The Notecard pulls them on its next inbound sync; no reflash, no truck roll. All are optional — firmware defaults apply when a variable is absent.
   ...
   |---|---|---|
   | `sample_interval_sec` | `120` | Seconds between sensor wakes. Min 30, max 3600. At 120 seconds the device takes 30 samples per hourly summary. |
...
![Data flow: sensors sampled every 120 seconds → six battery-condition rules plus one sensor-health alert → battery_alert.qo (sync:true) and battery_summary.qo (hourly templated) → Notehub → routes](diagrams/03-data-flow.svg)
...
A Notecarrier CX, a Cell+WiFi Notecard, an INA228 high-side voltage and current monitor, and a surface thermistor turn a passive cabinet backup battery into a continuously-monitored asset that reports float voltage, bidirectional current, surface temperature, and coulomb-counted state-of-charge every two minutes, and pages an operator within one sample interval of the site going dark.

For roadside cabinets, telecom huts, industrial RTU enclosures, and remote shelters, the cellular uplink is the whole point. The Notecard manages its own cellular session on a power path independent of the site's own modem and gateway, so the alert that the backup battery just failed reaches Notehub regardless of whether the site's primary radio is still up. The reporting tail extends through the cabinet battery's own discharge curve, through the DC-DC converter's lower input limit, and finally into the onboard LiPo, capturing the most diagnostically valuable minutes of a battery's life: the moments after mains drops and before the bus collapses.

The cost-versus-downtime calculus is straightforward. A single avoided emergency truck roll, or a single signalized intersection that does not go dark during a regional outage, pays for the sentinel hardware many times over, and the float-current trend in `battery_summary.qo` provides the leading indicator that lets a fleet plan scheduled swaps weeks before a battery would otherwise fail under load.

Everything else is extensibility. The same architecture and firmware, with the threshold recipe and shunt sized per chemistry and per current range, generalizes from a 12 V VRLA roadside cabinet to a 24 V LFP industrial UPS bank — and from one cabinet to a fleet, once `battery_summary.qo` is routed into a time-series store and ranked against the fleet baseline.
