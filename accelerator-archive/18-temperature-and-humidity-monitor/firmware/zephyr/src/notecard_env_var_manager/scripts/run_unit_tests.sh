...
if [[ ! -f "$ROOT_SRC_DIR/CMakeLists.txt" ]]; then
    echo "Failed to find notecard_env_var_manager root directory. (did the location of run_unit_tests.sh change?)"
    exit 1
fi
...
if [[ $COVERAGE -eq 1 ]]; then
    CMAKE_OPTIONS="${CMAKE_OPTIONS} -DNEVM_COVERAGE=1"
    BUILD_OPTIONS="${BUILD_OPTIONS} coverage"
fi
if [[ $MEM_CHECK -eq 1 ]]; then
    CMAKE_OPTIONS="${CMAKE_OPTIONS} -DNEVM_MEM_CHECK=1"
