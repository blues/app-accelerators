...
**Device-side responsibilities.** Each time the Notecard's ATTN signal brings the Cygnet STM32L433 host back to life, it does the same compact sample cycle: poll the APU controller over Modbus RTU, read both DS18B20 probes on the shared OneWire bus, and check the truck's ignition state on the voltage-divided analog input. The three-state idle-vs-APU state machine consumes those readings, the hourly and daily statistics get rolled forward in a struct that persists across sleep cycles in the Notecard's flash, and the cycle decides which of three Note types (if any) to emit — an hourly summary, a daily fuel-saved rollup, or an immediate fault alert. With the work done, the Cygnet calls [`NotePayloadSaveAndSleep`](https://dev.blues.io/api-reference/notecard-api/card-requests/#card-attn) to commit state to the Notecard and cut host power entirely for the next `sample_interval_sec` seconds via `card.attn`. Between samples the host MCU is completely dark.

**Notecard responsibilities.** From there the [Notecard for Skylo](https://dev.blues.io/datasheets/notecard-datasheet/note-nbglwx/) takes over. Each Note lands in onboard flash, every routine event ships on the [`hub.set`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) `outbound` cadence (default 60 minutes), and anything marked `sync:true` flushes immediately on whatever transport happens to be available. `card.transport` is configured to `"cell-ntn"` — cellular first, Skylo satellite fallback — so rural dead zones in Wyoming or Montana become brief delays in the hourly stream rather than data gaps. The Notecard also handles GNSS so each Note carries the position of the truck when the APU was running, and it distributes [environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) from Notehub the other way, letting the OEM retune fuel-rate assumptions and alert thresholds across the fleet without ever touching firmware.

**Notehub responsibilities.** [Notehub](https://dev.blues.io/notehub/notehub-walkthrough/) is where the data lands. The Notecard's embedded global SIM delivers events to it over the Internet; from there every event is stored and project-level [routes](https://dev.blues.io/notehub/notehub-walkthrough/#routing-data-with-notehub) fan them out to wherever the OEM's downstream system needs them. The three streams stay deliberately separate at the source — `apu_telemetry.qo` for hourly summaries, `apu_daily.qo` for once-per-day fuel-saved and runtime totals, and `apu_event.qo` for faults and temperature excursions — so each can fan out to a different destination at a different urgency without filter logic in the route. Routing destinations are intentionally left abstract; Notehub supports HTTP, MQTT, AWS, Azure, GCP, Snowflake, and others (see the [Notehub routing docs](https://dev.blues.io/notehub/notehub-walkthrough/#routing-data-with-notehub)).
...
Inside the sealed enclosure, everything traces back to the [Notecarrier CX](https://dev.blues.io/datasheets/notecarrier-datasheet/notecarrier-cx-v1-3/) and its dual 16-pin header. The Notecard for Skylo (NOTE-NBGLWX) seats into the carrier's M.2 slot, the RS-485 breakout and OneWire pull-up sit alongside, and the rest of the truck-side wiring — APU controller bus, ignition tap, two temperature probes, and the cab-roof antennas — comes back to that header. During bench validation, the Mojo sits inline between the 5 V DC/DC converter output and the Notecarrier's +VBAT pad so the sleep/wake current profile can be measured before any of this leaves the bench.
...
The firmware is split across three files so orchestration, shared declarations, and the Notecard/Modbus helpers each have a clear home:
...
This reference design is scoped to give an APU OEM the fastest credible path from "no connected product" to "live cellular-plus-satellite uplink with hourly summaries, daily rollups, and immediate fault alerts." A handful of details were deliberately left simple so the assembly-line story stays clean; each is documented below alongside the production extension that closes the gap.
...
For the APU OEM who used to ship mechanical hardware into the field and lose sight of it the moment the truck left the lot, every unit now reports back: hourly summaries, daily fuel-saved totals, and immediate fault alerts arrive in Notehub from wherever the truck happens to be at 2 AM — including the stretches of Wyoming where no single cellular carrier is reliable. One Notecard for Skylo handles global cellular, Skylo satellite fallback, GNSS, and the included data plan in a single M.2 module, and the Cygnet on the Notecarrier CX turns five Modbus registers and two temperature probes into the continuous, warrantyable stream that warranty databases and fleet analytics need. The fleet manager sees which trucks are actually using their APUs and for how long; the OEM sees the fault codes before the warranty return arrives; and both parties get the fuel savings pre-computed in each `apu_daily.qo` Note, ready to drop into a dashboard with no server-side aggregation. (`controller_runtime_hr` is the APU controller's cumulative lifetime runtime counter and must **not** be summed across Notes — treat it as the latest odometer reading.) The whole data stream lives independent of the fleet's own telematics vendor, independent of site WiFi, and independent of which carrier happens to have a tower over the next dark hundred miles.
