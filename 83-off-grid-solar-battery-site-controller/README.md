...
| Notecard cellular session (LTE-M outbound) | ~250 mA avg, ≤2 A peak, for ~15–30 seconds | same — radio dominates |
...
This reference design turns an off-grid solar and battery installation into a continuously monitored, remotely observable asset using a Notecarrier CX, a single Notecard, and the two Victron VE.Direct devices that are already installed at most off-grid sites. The Cygnet STM32 host wakes every 15 minutes, reads battery state-of-charge, current, temperature, and solar yield in a few seconds, evaluates four edge alert rules locally, and goes back to sleep — flushing a window of summary averages to Notehub every four hours and firing immediate alerts whenever the site is heading toward a hard-down condition.

For off-grid asset operators, the value is straightforward: a tower, cabin, or environmental monitoring station that quietly drifts toward a recharge deficit no longer goes dark unannounced. A `harvest_deficit` Note arrives in the NOC days before the bank is depleted, with enough context (consecutive deficit windows, average SoC, today's yield) to dispatch a truck before the outage rather than after.

The cellular plus Skylo connectivity story matches the deployment reality of these sites. Where terrestrial LTE-M reaches, the Notecard uses it and the included global SIM keeps operating costs flat. Where it does not — the wilderness arrays, remote towers, and high-altitude installations that benefit most from this monitoring — the same firmware and the same M.2 carrier accept a Notecard for Skylo and reach Notehub over the NTN satellite network, with no carrier negotiation, no separate billing relationship, and no second firmware build.

The same pattern extends naturally beyond solar and battery monitoring — anywhere a Victron VE.Direct device, a Modbus-RTU sensor, or a simple analog signal needs to reach the cloud from a site that has no IT infrastructure of its own.
