...
Then, enter this command to configure the Notecard's accelerometer's sensitivity (an integer, ranging from -1 to 5):

```json
{ "req": "card.motion.mode", "sensitivity": 1 }
```

The lower the sensitivity value, the less sensitive the accelerometer will be. We only want to detect drastic movements due to a collision, so we are setting the value low. Setting sensitivity to -1 is probably a better choice for the real-world application, but a sensitivity of 1 is better for local testing.
...
The `"start": true` parameter tells the Notecard to begin motion tracking.
