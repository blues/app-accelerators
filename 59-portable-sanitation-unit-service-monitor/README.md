...
**Device-side responsibilities.** Every five minutes the onboard Cygnet STM32 host on the Notecarrier CX wakes, reads the ultrasonic probe, folds the reading into the running window, and asks one question: is the tank above the service trigger? Between those brief wakes the host is fully powered off by [`card.attn`](https://dev.blues.io/api-reference/notecard-api/card-requests/#card-attn), with only the Notecard's low-power idle circuit (~8–18 µA) keeping time. Queued [Notes](https://dev.blues.io/api-reference/glossary/#note) travel from the host to the Notecard over I²C — no JSON marshaling, no serial buffer management, no modem AT commands to babysit. A mandatory `ATTN`-to-`EN` jumper on the Notecarrier CX header gates the host's power supply and is **required** for that sleep model to actually save any battery; see §6.

**Notecard responsibilities.** The Notecard parks each [Note](https://dev.blues.io/api-reference/glossary/#note) in its on-device queue, opens a cellular (or WiFi) session on the configured [`hub.set`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) `outbound` cadence (default every 60 minutes), and flushes any `sync:true` alert Note immediately regardless of that schedule — the dispatcher hears about a full tank in the same minute it happens, not at the top of the next hour. The Notecard also pulls [environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) from Notehub on each inbound sync, so tank calibration values and service thresholds propagate to every unit in the fleet without anyone reflashing firmware.

**Notehub responsibilities.** Each unit's embedded global SIM lets the Notecard authenticate against any supported carrier worldwide and deliver data to [Notehub](https://notehub.io) over the Internet — no carrier paperwork, no per-unit provisioning. Notehub stores every event and applies project-level [routes](https://dev.blues.io/notehub/notehub-walkthrough/#routing-data-with-notehub). Summaries and alerts arrive in separate [Notefiles](https://dev.blues.io/api-reference/glossary/#notefile), so a dispatcher's real-time queue and an analyst's billing warehouse can be served from the same device without any per-Note filter logic in between. [Fleets](https://dev.blues.io/guides-and-tutorials/fleet-admin-guide/) and [Smart Fleets](https://dev.blues.io/notehub/notehub-walkthrough/#using-smart-fleet-rules) group units by region, client, or tank size — a different calibration profile pushes to the 75-gallon ADA fleet without touching the 60-gallon standard units sitting beside them.
...
<!-- TODO: verify Taoglas FW.10.0113 — part number unconfirmed on Taoglas/Mouser; consider Taoglas FXUB63 or FW.10.A.301111 as alternatives -->
...
The whole assembly is small enough to live in a 120 × 90 mm enclosure: a Notecarrier CX with the Notecard Cell+WiFi seated in its M.2 slot, every host I/O landing on the carrier's dual 16-pin header, and a single sealed probe cable exiting through a cable gland to the tank fitting. During bench power validation the Mojo sits inline between the LiPo's JST-PH connector and the Notecarrier CX's `+VBAT` pin (see [§9](#9-validation-and-testing)); it comes out before the unit ships to a job site.
...
The whole firmware lives in a single sketch — [`firmware/sanitation_unit_monitor/sanitation_unit_monitor.ino`](firmware/sanitation_unit_monitor/sanitation_unit_monitor.ino) — because the device only has one job: read a sensor, decide whether to alert, and go back to sleep.
...
This reference design is deliberately scoped to one job — measure a tank, decide whether to send a truck, and survive on a single LiPo between service visits. Several things a production fleet would care about have been left for the integrator to add; they aren't oversights so much as boundaries that keep the POC focused.
...
The dispatcher who used to guess now opens a map of units ranked by fill percentage, sends a truck to the two that are at 80%, and leaves the other eight alone — exactly the outcome that the phrase "truck roll reduction" describes. The Notecard Cell+WiFi handles the connectivity half of the problem with a single SKU that follows each unit to every new job site and never needs to be re-provisioned; the Notecarrier CX with its deep-sleep host handles the power half by drawing almost nothing between hourly check-ins. Notehub holds the fleet-management half together, letting an operator retune thresholds and tank geometry from a browser without touching a single enclosure. The bench/POC assembly uses a MaxBotix MB7389 to prove out the full firmware and data flow; a field deployment swaps in an intrinsically safe sensor selected for the applicable hazardous-area classification (see §4 and §11). Satellite Industries built their commercial product along exactly these lines — this reference project gives the next service company or integrator the same starting point.
