...
`sync:true` tells the Notecard to bypass the outbound timer and open a cellular session now. The dispatcher sees the alert within a typical LTE-M session-establishment window (~15–60 seconds).
...
| Notecard cellular session — LTE-M, hourly, ~30 seconds typical | ~250 mA average; up to ~2 A peak supply bursts during radio registration (see [NOTE-MBGLW datasheet](https://dev.blues.io/datasheets/notecard-datasheet/note-mbglw/) and [low-power design guide](https://dev.blues.io/notecard/notecard-walkthrough/low-power-firmware-design/)) | Cumulative per-session mAh increment visible as a step in `card.power` output and in `_log`/`power` per-session deltas. Mojo does **not** resolve instantaneous supply bursts — use an oscilloscope or inline current probe for peak-current validation. |
