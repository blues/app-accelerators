...
#define DOOR_PORT  A1_GPIO_Port
#define DOOR_PIN   A1_Pin
#define DOOR_IRQN  EXTI2_IRQn
...
bool dsmInit()
{
    ...
    HAL_GPIO_DeInit(DOOR_PORT, DOOR_PIN);
    GPIO_InitTypeDef gpioInit = {0};
    gpioInit.Pin = DOOR_PIN;
    ...
    HAL_GPIO_Init(DOOR_PORT, &gpioInit);
    HAL_NVIC_SetPriority(DOOR_IRQN, 15, 0);
    HAL_NVIC_EnableIRQ(DOOR_IRQN);
...
static void dsmPoll(int appID, int state, void *appCtx)
{
    bool doorOpen = HAL_GPIO_ReadPin(DOOR_PORT, DOOR_PIN);
