...
![Creek / Culvert Flood-Warning Stream Gauge banner](banner.png)
...
**Device-side responsibilities.** Every five minutes the Cygnet STM32 host on the Notecarrier CX comes out of [`card.attn`](https://dev.blues.io/api-reference/notecard-api/card-requests/#card-attn) sleep and does a full sample cycle before it powers back down: it averages three MB7389 distance readings over the UART, counts rain-gauge reed-switch closures during a 3-second observation window, updates the 8-entry rolling history that drives the rate calculation, walks the four threshold conditions in priority order, and emits at most one alert Note per wake for the highest-priority condition that trips. All Notecard traffic uses I²C, which keeps the Cygnet's hardware UART free for the MB7389 sensor.

**Notecard responsibilities.** Once the host has dropped its sample into Notecard flash, the Notecard takes over. It manages the cellular or satellite session on the [`hub.set`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) `periodic` outbound cadence (default 60 minutes) and, when an alert is marked `sync:true`, fires the radio immediately on whatever transport — cellular or Skylo satellite — is currently available (the firmware configures `card.transport method=cell-ntn`; WiFi credentials are never provisioned). The inbound poll cadence is **decoupled from outbound** and defaults to 24 hours (`inbound_interval_min`) for a reason: each inbound session is its own radio transaction, and every NTN session nibbles at the 10 KB satellite budget. By spacing inbound polls far apart, the device reserves its satellite minutes for the data that matters — alerts and summaries — instead of routine env-var fetches. [Environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) pushed from Notehub reach the device on the next inbound poll, which is fine for a static field installation where thresholds rarely change. Shorten `inbound_interval_min` only when rapid remote reconfiguration is genuinely required, and account for the added NTN sessions in your satellite budget.

**Notehub responsibilities.** [Notehub](https://notehub.io) takes everything from there: it ingests every event, stores it, and applies project-level routes. The Notecard manages its own cellular and Skylo NTN satellite sessions against supported carrier networks worldwide and delivers data to Notehub over the Internet. Alerts and summaries land in separate [Notefiles](https://dev.blues.io/api-reference/glossary/#notefile) on purpose — so a road-closure alert can be fanned out to an emergency-dispatch or public-works on-call system, while the hourly readings flow into a long-term hydrology data store on a different schedule entirely.
...
Inside the IP67 enclosure, everything funnels back to the [Notecarrier CX](https://dev.blues.io/datasheets/notecarrier-datasheet/notecarrier-cx-v1-3/) and its dual 16-pin header. The Notecard for Skylo seats into the carrier's M.2 slot, the Adafruit 4755 charger rides alongside the Notecarrier, and the 6600 mAh LiPo plugs into the charger's JST battery port — with the charger's JST load output feeding the Notecarrier's +VBAT pad.
...
The firmware lives in `firmware/creek_flood_gauge/` and is split across three files so that the orchestration logic, the shared state, and the sensor- and Notecard-facing helpers each have a clear home:
...
This reference design optimizes for a deployable POC that proves out the trend-based alerting story end-to-end. A handful of details were intentionally left simple so that a county engineer could stand up the first gauge in an afternoon — each of them is documented below alongside the production path.
