...
**What you'll have when you're done:** a weatherproof, trailer-powered monitor that samples cargo temperature every minute, queues each sample locally for Notehub delivery on the next outbound session (`trailer_log_cell.qo`, arriving in Notehub in batches, default once per hour), fires an alert on the next sample cycle (up to ~60 seconds after the event, plus network-establishment time) when a door opens or a temperature threshold is breached, and ships hourly compliance summaries to Notehub over cellular or WiFi. Per-sample logs and hourly summaries use non-NTN-compatible Notefiles; the Notecard discards their queued Notes when connecting via NTN, so they never consume the bundled satellite data budget. On periodic hub sessions over NTN — which occur when terrestrial coverage is unavailable — only pending alert Notes carry payload data over the satellite link.
...
This project pairs a Notecarrier CX and a Notecard for Skylo with two DS18B20 temperature probes and a magnetic reed switch to deliver a weatherproof, trailer-powered monitor that catches the two failure modes responsible for most refrigerated cargo loss: temperature excursion and unauthorized door access.

For shippers operating under FSMA Sanitary Transportation rules or maintaining HACCP records, and for pharmaceutical cold-chain operators governed by GDP and per-shipment release criteria, the per-sample log and hourly summary Notes provide the continuous, independently-collected condition record that audits and freight claims demand. The compact, geo-stamped alert Notes capture the door and excursion events that matter for real-time response.

Dual-network connectivity (cellular plus Skylo NTN satellite) is the load-bearing design choice for transcontinental routes. A reefer trailer crosses dense urban cellular, rural-interstate gaps, port staging yards outside terrestrial coverage, and cross-border carrier handoffs over the course of a single haul. The `wifi-cell-ntn` policy lets the Notecard pick the right radio at each moment without firmware changes, and the cellular-only Notefiles for per-sample logs and hourly summaries are discarded at sync time when NTN is the active transport, keeping the bundled satellite data budget intact while real-time alerts still ride the satellite link.

The same firmware skeleton extends naturally to shipper-specific use cases: pharma cold chain, cross-border intermodal containers, frozen-food long-haul, or any mobile insulated enclosure where a temperature log and a door audit trail carry regulatory or financial weight.
