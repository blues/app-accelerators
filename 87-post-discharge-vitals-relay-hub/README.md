...
**What you'll have when you're done:** a wall-powered hub that sits on the patient's nightstand, continuously scans for the four standard BLE health-device service profiles, parses each measurement from the raw GATT (Generic Attribute Profile) characteristic bytes, queues it in a type-specific Notefile, and uploads to Notehub every 15 minutes. Readings that cross configurable clinical thresholds bypass the upload timer and trigger an immediate cellular sync — so the care team sees an alarming blood pressure or a precipitous weight gain within seconds, not on the next scheduled check-in.
...
The rest of this README expands each step and explains why the firmware is shaped the way it is.
...
| [Notecard Cell+WiFi (MBGLW)](https://dev.blues.io/datasheets/notecard-datasheet/note-mbglw/) | 1 | Cellular removes the WiFi dependency for the patient; no credential provisioning needed. Ships with a global prepaid SIM — no activation, no monthly commitment per device. |
...
- **`_session.qo`** — appears within a minute of first power-on. Confirms the Notecard has cellular coverage and your ProductUID is correct.
...
- **`vitals_alert.qo`** — emitted alongside the threshold-tripping measurement note, also with `sync:true`. Both notes typically arrive in the same immediate cellular session (~15–60 s), or in back-to-back immediate sessions — bypassing the normal 15-minute upload timer.
- **`commissioning.db`** — written during a commissioning build (both `ALLOW_UNENROLLED_DEVICES_FOR_DEV=1` and `ALLOW_COMMISSIONING_BUILD` active) when the hub successfully bonds a new device. Each `bond_established` note contains the device's `ble_addr` (colon-separated, MSB-first) and `addr_type`. Inspect these notes after commissioning: devices with `addr_type` `0x00` (public) or `0x01` (random static) must be added to `ENROLLED_DEVICES` in `vitals_config.h` before the shipping build — see the **Commission** step in the quickstart at the top of this document and §9 Limitations.
...
- **Blues Wireless Notecard** (`note-arduino`) — install via the Arduino Library Manager (search "Blues Wireless Notecard"), current stable release **v1.8.5** at time of writing. Check [github.com/blues/note-arduino/releases](https://github.com/blues/note-arduino/releases) for the latest release before flashing.
...
The hub is USB wall-powered, so deep sleep is neither necessary nor desirable — the BLE scanner must remain active to catch patient-initiated measurements. The Notecard runs in `periodic` mode with `outbound:15` (minutes) and `inbound:60`. Readings queue on the Notecard between cellular sessions and upload in a single burst every 15 minutes. When a reading trips a threshold, `sync:true` is added to both the measurement note (e.g., `bp.qo`) and the companion `vitals_alert.qo` note — both bypass the queue and are typically delivered in the same immediate cellular session, or in back-to-back immediate sessions. The 15-minute default is conservative — for most RPM programs, readings uploading within 15 minutes of being taken is well inside the care-team's response window.

**Alert cooldown.** Once a `vitals_alert.qo` note fires for a given alert type, the firmware suppresses further alert notes of the same type for `ALERT_COOLDOWN_MS` (5 minutes). The threshold-tripping measurement note still carries `sync:true` during the cooldown window — the care team's data record remains complete and arrives immediately — but the companion alert note (which may trigger a page or automated care-team response) is held back. A reading that remains out of range across multiple consecutive measurements therefore produces at most one alert every 5 minutes rather than one per reading cycle. The 5-minute floor is a firmware constant in this POC; a production deployment would expose it as a configurable Notehub environment variable alongside the clinical thresholds.

The nRF52840 SoftDevice duty-cycles the BLE radio automatically: the scanner runs at a 50% duty cycle (100 ms window every 200 ms interval). The BLE scanner dominates the continuous baseline current draw (~5–10 mA); each cellular session dominates the short peak-current burst (~250 mA) and contributes a significant share of total energy per 15-minute sync window despite being brief (~15–30 s). No host sleep / wake cycle is implemented.
...
| Alerts fire on every reading even though values look normal. | A threshold explicitly set to `0`, stale cached env values, or a scope-override shadowing the intended value. | In Notehub, verify all six threshold env vars have non-zero values — a literal `"0"` applies as zero and will trip every reading. Blank strings are guarded in firmware (`atof("")` is blocked before application) and do not become zero. Notehub applies env vars hierarchically: project scope is the baseline, fleet scope overrides it, and device scope overrides both. A device-level `"0"` value silently shadows a fleet or project default, so check all three scopes when a threshold behaves unexpectedly. |
