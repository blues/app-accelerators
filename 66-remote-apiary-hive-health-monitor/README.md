...
1. **Scale-factor calibration.**
   ...
   The HX711 library computes `get_units() = raw_counts / scale`, so increasing the scale factor lowers the reported value and vice versa: if the reported net weight is **too high**, the ratio exceeds 1 and `new_calibration` increases (brings the output down); if too low, the ratio is < 1 and  ED272  decreases (brings the output up).

   **Example:**  ED273 ,  ED274 ,  ED275  with 10 kg reference → net reported = 13.0 kg (30 % high) →  ED276 . Set  ED277  in Notehub, force an inbound sync, then set  ED278  and force a second inbound sync to discard the current partial window (which contains readings taken under the old calibration factor). Run one more full-window known-mass test with the platform load undisturbed to confirm the result is within ±0.5 kg before proceeding. The reset fires once; you may leave  ED279  in place or remove it — no further action is needed to prevent repeated resets.

2. **Platform tare (zero offset).**

   a. After scale-factor calibration is confirmed, remove the reference mass and leave only the empty stand platform undisturbed for one full summary window. Record  ED280  from that note — this is the platform dead-load.

   b. Set  ED281  in Notehub to that value, force an inbound sync, then set  ED282  and force a second inbound sync to start a clean verification window. From this point, all  ED283  readings represent hive body and contents only (referenced to zero with the empty stand). Confirm the first full-window empty-platform summary reports approximately 0.0 kg (typically within ±0.2 kg for this load cell class). The reset fires once; you may leave  ED284  in place or remove it — subsequent wakes will not repeat the reset.
