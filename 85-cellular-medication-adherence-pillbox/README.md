...
**Notecard responsibilities.** The Notecard Cell+WiFi (NOTE-MBGLW) stores Notes locally in its on-device queue, manages the cellular (or WiFi) session on the configured [`hub.set`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) `periodic` outbound cadence, and flushes any `sync:true` Notes immediately regardless of the outbound timer. The Notecard also distributes [environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) from Notehub to the host — a care coordinator can update the poll interval, daily summary hour, or sync cadences without reflashing firmware. When the host is sleeping, the Notecard idles at roughly 8–18 µA, making it effectively invisible on the LiPo budget between cellular sessions.

**Notehub responsibilities.** [Notehub](https://dev.blues.io/notehub/notehub-walkthrough/) terminates the cellular session, stores every event, and applies project-level routes. `pill_open.qo` events land in near-real time for clinician review; `pill_summary.qo` notes accumulate for trend analysis. [Fleets](https://dev.blues.io/guides-and-tutorials/fleet-admin-guide/) can group devices by care coordinator, practice, or patient tier, with per-fleet environment variables applied to all devices in the group at once.
...
| [Notecard Cell+WiFi (NOTE-MBGLW)](https://dev.blues.io/datasheets/notecard-datasheet/note-mbglw/) | 1 | Cellular with WiFi fallback — see the [NOTE-MBGLW datasheet](https://dev.blues.io/datasheets/notecard-datasheet/note-mbglw/) for radio and power specifications. Cellular removes all patient-side network configuration; cellular-first is the deployment model that serves the target demographic. |
