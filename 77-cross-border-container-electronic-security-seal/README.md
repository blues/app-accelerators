...
![Cross-Border Container Electronic Security Seal banner](banner.png)
...
**Device-side responsibilities.** The Blues Swan STM32 host on the Notecarrier XI is the brain inside the seal. Every 30 seconds (configurable via environment variable) it wakes, reads the door reed switch on GPIO A0 and the break-wire continuity loop on GPIO A1, and asks one question of each: has anything changed? If either has, the host signs the event payload using the ATECC608A — a hardware secure element on the shared I²C bus whose private key never leaves the chip — and queues a `seal_event.qo` [Note](https://dev.blues.io/api-reference/glossary/#note) with `sync:true` and the truncated `sig` field, plus a companion `seal_sig_full.qo` Note carrying the full 64-byte signature for cellular delivery. Because the seal-wire check fires independently of door state, a cut-and-replaced seal before any door opening still produces its own timestamped, signed record — exactly the case a door-state-only monitor would miss. Every six hours by default the host also signs and builds a `seal_heartbeat.qo` Note with door state, seal-wire state, battery voltage, and `sig`; the Notecard's GPS fix populates the location fields automatically via the compact template's `_lat`/`_lon` reserved fields. (Audit-gap indicator Notes are deliberately unsigned — they're chain-of-custody gap markers, not verifiable events.) Between wakes the host is fully powered off via [`card.attn`](https://dev.blues.io/api-reference/notecard-api/card-requests/#card-attn), pulling essentially zero current from the battery rail.

**Notecard responsibilities.** The Notecard Cell+WiFi is the connectivity layer that hides every network decision from the host. It holds [Notes](https://dev.blues.io/api-reference/glossary/#note) in its on-device flash queue, manages the cellular modem, coordinates with the Starnote for satellite sessions, and flushes Notes to Notehub on the configured [`hub.set`](https://dev.blues.io/api-reference/notecard-api/hub-requests/#hub-set) outbound cadence. Notes flagged `sync:true` — every breach event — bypass that timer and trigger an immediate connection attempt. The choice between LTE cellular and Iridium satellite via the Starnote is autonomous and based on what's available; the host never touches the modem. The Notecard also handles GPS acquisition on a six-hour periodic schedule and distributes [environment variables](https://dev.blues.io/guides-and-tutorials/notecard-guides/understanding-environment-variables/) from Notehub down to the host so cadences and thresholds can be tuned without a firmware reflash. When `outbound_min` or `inbound_min` changes in Notehub, the firmware re-applies `hub.set` on the next wake cycle after the env-var refresh.

When neither satellite nor cellular is reachable — below-deck stowage, containers buried deep in a stack, terminal sheds — Notes accumulate in the Notecard's flash queue, durable across power cycles. When sky view returns, the Notecard flushes the accumulated queue in the next successful session. Voyage stowage means that blackout gap can span hours to days, so breach-alert latency and waypoint density should be sized with that in mind from the start.

**Notehub responsibilities.** Once Notes leave the device, the Notecard's embedded global SIM carries them over supported carriers worldwide and delivers them to [Notehub](https://notehub.io). Notehub ingests every event with device identity and reception timestamp, stores it, and applies project-level routes. Splitting the two Notefiles at the source — `seal_event.qo` for door events, `seal_heartbeat.qo` for waypoints — lets you fan them out to different downstream endpoints via [Notehub routes](https://dev.blues.io/notehub/notehub-walkthrough/#routing-data-with-notehub): a compliance database for the heartbeats, an alerting webhook for the breach events.
...
| No `seal_event.qo` within 60 seconds of manually triggering a breach. | In port (cellular) — should arrive quickly. At sea (satellite) — delivery depends on sky visibility; Iridium LEO sessions typically establish within a few minutes with a clear sky view; obstruction can delay them. | In a bench test over cellular, a `seal_event.qo` should appear within ~60 s. If longer, check signal with `card.status` in the blues.dev In-Browser Terminal. |
...
A tamper-evident seal in production has to defend against an adversary, not just produce a clean event log on a bench. The list below identifies where this reference design draws the scope line: the trade-offs that make the POC implementable in one Notecarrier without sacrificing the core forensic property (hardware-signed events delivered globally), and the places a real chain-of-custody deployment will need to harden.
...
The dispute over who opened a container, and when, used to be a paper trail that everyone could plausibly deny. With this seal, that dispute is now a hardware-signed event log — every seal-wire break, door breach, re-seal, and GPS waypoint from first loading to final delivery, each one signed by a P-256 private key that never leaves the ATECC608A and transmitted over whatever network is available: LTE-M in the terminal, Iridium satellite anywhere at sea including polar routes, and back to LTE-M on the inland leg. The Swan wakes every 30 seconds to read its two sensors, signs anything new, and goes back to sleep at a fraction of a milliamp; the Notecard idles at 18 µA between flushes; `sync:true` events bypass the outbound schedule and reach Notehub within seconds in port or within minutes over Iridium anywhere on the globe. For the customs broker, the ocean carrier, and the compliance auditor, the chain of custody is no longer a story told after the fact — it's a verifiable record produced in real time, from a seal assembly, a battery, and two antennas pointed at the sky.
