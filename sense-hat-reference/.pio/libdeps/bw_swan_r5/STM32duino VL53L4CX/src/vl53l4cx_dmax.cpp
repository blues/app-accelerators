...
VL53L4CX_Error VL53L4CX::VL53L4CX_f_001(
  ...
  int16_t                              *pambient_dmax_mm)
{
  ...
  pdata->VL53L4CX_p_004     = 0x0000;
  pdata->VL53L4CX_p_033 = 0x0000;
  pdata->VL53L4CX_p_034          = 0x0000;
  pdata->VL53L4CX_p_009    = 0x0000;
  pdata->VL53L4CX_p_028     = 0x0000;
  pdata->VL53L4CX_p_035 = 0x0000;
  pdata->VL53L4CX_p_036             = 0;
  pdata->VL53L4CX_p_022            = 0;

  *pambient_dmax_mm  = 0;


  if ((pbins->VL53L4CX_p_015        != 0) &&
      (pbins->total_periods_elapsed      != 0)) {



    pll_period_us   =
      VL53L4CX_calc_pll_period_us(pbins->VL53L4CX_p_015);
    ...
    pdata->VL53L4CX_p_034 =
      VL53L4CX_rate_maths(
        pbins->VL53L4CX_p_028,
        pdata->VL53L4CX_p_037);



    pdata->VL53L4CX_p_033   =
    ...
    pdata->VL53L4CX_p_038 = pcfg->max_effective_spads;
    pdata->VL53L4CX_p_004  = pcfg->max_effective_spads;

    if (pdata->VL53L4CX_p_033 > 0) {
      tmp64   =
        (uint64_t)pcfg->dss_config__target_total_rate_mcps;
      tmp64  *= 1000;
      tmp64 <<= (11 + 1);
      tmp32 = pdata->VL53L4CX_p_033 / 2;
      tmp64 += (uint64_t)tmp32;
      tmp64 = do_division_u(tmp64,
                            (uint64_t)pdata->VL53L4CX_p_033);

      if (tmp64 < (uint64_t)pcfg->max_effective_spads) {
        pdata->VL53L4CX_p_004 = (uint16_t)tmp64;
      }
    }
  }
  ...
  if ((pcal->ref__actual_effective_spads != 0) &&
    ...
    pdata->VL53L4CX_p_009   = (uint32_t)tmp64;
    pdata->VL53L4CX_p_009 <<= 4;



    tmp64   = (uint64_t)pdata->VL53L4CX_p_037;
    tmp64  *= (uint64_t)pdata->VL53L4CX_p_033;
    tmp64  *= (uint64_t)pdata->VL53L4CX_p_004;
    tmp64  += (1 << (11 + 7));
    tmp64 >>= (11 + 8);
    tmp64  +=  500;
    ...
    pdata->VL53L4CX_p_028     = (uint32_t)tmp64;



    tmp64   = (uint64_t)pdata->VL53L4CX_p_037;
    tmp64  *= (uint64_t)pdata->VL53L4CX_p_009;
    tmp64  *= (uint64_t)pdata->VL53L4CX_p_004;
    tmp64  += (1 << (11 + 7));
    tmp64 >>= (11 + 8);
    ...
    pdata->VL53L4CX_p_035 = (uint32_t)tmp64;



    tmp32  = VL53L4CX_isqrt(pdata->VL53L4CX_p_028 << 8);
    tmp32 *= (uint32_t)pcfg->ambient_thresh_sigma;



    if (pdata->VL53L4CX_p_028 <
        (uint32_t)pcfg->min_ambient_thresh_events) {

      amb_thres_delta =
        pcfg->min_ambient_thresh_events -
        (uint32_t)pdata->VL53L4CX_p_028;
    ...
    pdata->VL53L4CX_p_022 =
      (int16_t)VL53L4CX_f_002(
        tmp32,
        pdata->VL53L4CX_p_035,
        (uint32_t)pcal->ref__distance_mm,
        (uint32_t)pcfg->signal_thresh_sigma);



    tmp32  = (uint32_t)pdata->VL53L4CX_p_035;
    tmp32 *= (uint32_t)pbins->vcsel_width;
    tmp32 += (1 << 3);
    tmp32 /= (1 << 4);

    pdata->VL53L4CX_p_036 =
      (int16_t)VL53L4CX_f_002(
    ...
    if (pdata->VL53L4CX_p_036 < pdata->VL53L4CX_p_022) {
      *pambient_dmax_mm = pdata->VL53L4CX_p_036;
    } else {
      *pambient_dmax_mm = pdata->VL53L4CX_p_022;
    }
...
uint32_t VL53L4CX::VL53L4CX_f_002(
  ...
  uint32_t     signal_thresh_sigma)
{
  ...
  tmp32  = VL53L4CX_isqrt(tmp32);
  tmp32 += (uint32_t)signal_thresh_sigma;



  range_mm =
    (uint32_t)VL53L4CX_isqrt(ref_signal_events << 4);
