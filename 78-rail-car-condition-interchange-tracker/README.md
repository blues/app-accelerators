...
| Sample cycle: host + sensors active (~5–10 seconds) | — | — | ~30–40 mA |
| Cellular sync — average current | Modem active: ~250 mA | [NOTE-NBGLWX datasheet](https://dev.blues.io/datasheets/notecard-datasheet/note-nbglwx/) | ~150–300 mA average; session ~15–60 seconds typical |
...
|---|---|---|
| All-cellular, mostly-stationary (car in yard) | 15–25 mAh / 24 h | GNSS motion-gated off. Host sleeps 15 minutes / wake cycle for ~5–10 s. Sync every 4 h at normal charge. **Best-case baseline for yard-dwell scenarios.** |
| All-cellular, in-transit (car moving hours/day) | 40–80 mAh / 24 h | GNSS active every 5 minutes while moving. `railcar_location.qo` fires every 30 minutes + on motion edges. GNSS duty cycle adds significant budget. Validate empirically with Mojo under representative duty cycle. |
| Mixed NTN/cellular or predominantly NTN | 80–200+ mAh / 24 h | NTN session overhead dominates. Sessions last 1–4 minutes at high current; exact budget depends on coverage in your corridor. **Measure with Mojo in your target deployment zone before sizing.** |
...
- **Frequent satellite sessions:** very long (1–4 minutes) high-current bursts replacing the shorter cellular bursts. Normal behavior when the car is in a cellular-dark corridor.
